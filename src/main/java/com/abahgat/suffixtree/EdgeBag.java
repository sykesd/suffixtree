...
/**
 * A specialized implementation of Map that uses native int types and sorted
 * arrays to minimize the memory footprint.
...
class EdgeBag implements Map<Integer, Edge> {
    ...
    @Override
    public Edge put(Integer codePoint, Edge e) {
        if (!Character.isValidCodePoint(codePoint)) {
            throw new IllegalArgumentException("Illegal input code point: " + codePoint + ".");
        }
    ...
    @Override
    public Edge get(Object maybeInteger) {
        return get(((Integer) maybeInteger).intValue());  // throws if cast fails.
    }

    public Edge get(int codePoint) {
        if (!Character.isValidCodePoint(codePoint)) {
            throw new IllegalArgumentException("Illegal input code point: " + codePoint + ".");
        }
    ...
    private int search(int codePoint) {
        ...
        if (codePoints.length > BSEARCH_THRESHOLD) {
            return java.util.Arrays.binarySearch(codePoints, codePoint);
        }
